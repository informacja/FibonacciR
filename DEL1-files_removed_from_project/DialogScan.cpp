...
__fastcall TSearchPref::TSearchPref(TComponent* AOwner)
	: TForm(AOwner)
{
EditPath->Text = FibonacciR->EditSciezkaszukana->Text;
...
if (SelectDirectory("Otwórz", String("2").w_str(), Dir,
   TSelectDirExtOpts() << sdNewUI << sdNewFolder << sdShowShares << sdShowFiles))
	EditPath->Text = Dir;
	FibonacciR->EditSciezkaszukana->Text = Dir;
}
//---------------------------------------------------------------------------
...
{
this->Hide();
FibonacciR->EditSciezkaszukana->Text = EditPath->Text;
FibonacciR->LabelEXT->Caption = ComboBoxEXT->Text;
FibonacciR->filter = ComboBoxEXT->Text;
...
{
	FibonacciR->filter = ComboBoxEXT->Text;
	ComboBoxEXT->Hint = FibonacciR->filter;
}
//---------------------------------------------------------------------------
