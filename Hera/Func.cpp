...
// ---------------------------------------------------------------------------
 ...
 if (DirectoryExists(path)) {
		...
		if ( b ) {      // czy ma byæ backslash na koñcu
		...
		{
			if ( LastDelimiter('\\', path) == path.Length()) {
...
bool Copyfile2(String e_path, String n_path) {

   if ( !FileExists(e_path,1)) {

//	ImageList1->GetIcon(4, ImageSave->Picture->Icon);                           // ikona powodzenia

	   if( ExistsDir( e_path ))

			ShowMessage("Foldery nie s¹ obs³ugiwane");
//			throw Exception("Foldery nie s¹ obs³ugiwane");

	   else ShowMessage("Plik nie istnieje");
//	   throw Exception("Plik nie istnieje");
   }


	 while (FileExists(n_path ) ) {                    			// inkrementacja nazwy pliku

		n_path = IncremFileName(n_path);
	}
...
//	 StatusInfo( Path + ":" + FileNameB);                             // œcie¿ka na statusBarze po inkrementacji
	...
	try {
	   ...
	   if( !CopyFileExW( e_path.c_str(), n_path.c_str(), 0,0,0,0))// w przypadku niepowozenia
		{
//			  ShowMessage(GetLastError());
//			ImageList1->GetIcon(5, ImageSave->Picture->Icon);                   // ikona niepowodzenia operacji
			ShowMessage(ErrorToText(GetLastError(), 1,1));                       // kod b³êdu

		} else ShowMessage(GetLastError()); //ImageList1->GetIcon(3, ImageSave->Picture->Icon);               // ikona powodzenia
...
ShowError("(for programmer)lastreslut zarejestrowano w DigitalRewrite \n0: " + lastreslut[0] +" \n1: "+ lastreslut[1]);
...
AnsiString GetSize(float SizeInByte, bool Bits) {
	...
	else return FormatFloat("0.0#", SizeInByte) + " b";                         //bity
	...
	*buff = (Bits) ? "b" : "B";
	  ...
	  return FormatFloat("0.0#", SizeInByte) + " " + *buff;
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
void QuestSearchMoreThan(TListView *lista, String *Dir, int *c)
{
	S::T.Pause();
	lista->Show();
	S::Status("Ostatni plik: " + *Dir + " (" + *c + ")");
	...
	switch ( MessageBoxA(Application->Handle, S::BreakQuest().c_str(),
		...
		case idYes:   lista->Hide(); S::T.Continue(); break;
		case idNo:
S::T.Continue();
	 S::T.Show();

		throw Exception("Przerwanie"); break;  //bug                               // w trybie debug to wlicza siê do pomiaru czasu
...
// ---------------------------------------------------------------------------
 String Search(TListView *lista, String *Dir, String typ, unsigned n, unsigned char m) {    // n to stan licznika przy wywo³ywaniu rekurencyjnym
	...
	if ( m > S::MAX_DEPTH ) return "Too depth";
	...
	if ((((*sr).Attr & faDirectory) > 0) && ((*sr).Name != ".") && 	((*sr).Name != ".."))
			  ...
			  Search(lista, &(Getpath(*Dir) + (*sr).Name) , typ, (*c).get(), m + 1); // przeszukiwanie podfolderów
				...
				if (((*sr).Attr & faDirectory) == 0)
				{
					if (typ.IsEmpty())
					{
						ShowError("Zg³oœ do b³¹d autorowi programu. Pomoc->Poinformuj... \nEmpty search() " + (*c).get() );
						ListIt = lista->Items->Add();
						ListIt->Caption = *Dir;
						ListIt->SubItems->Add((*sr).Name);
						(*c).inkrement();
					}
//					else if ( CompareExtension( (*sr).Name, typ) ) // nie ma potrzeby sprawdzania ponownie typu, funkcja FindFirst ju¿ to robi
						else
					{
//						ShowMessage(ExtractFileExt((*sr).Name));
						 ListIt = lista->Items->Add();
						 ListIt->Caption = Getpath( *Dir, false );
						 ListIt->SubItems->Add((*sr).Name);
						 ListIt->SubItems->Add(GetSize((*sr).Size));
						 ListIt->SubItems->Add( (*sr).Size );                   // do póŸniejszego wyliczania i sortowania informacja w bajtach
						 Counter::size_files  += (*sr).Size;
						 ListIt->ImageIndex = TypeFileIcon( &( Getpath(*Dir,1) + (*sr).Name), lista);
//						ListIt->SubItems->i = TypeFileIcon( &( Getpath(*Dir,1) + (*sr).Name), lista);
						 (*c).inkrement();
					}

					if ((*c).get() == S::BREAK_SEARCH)

						QuestSearchMoreThan( lista, Dir,(int*) &c->size_files );
				}
		}
		while (FindNext(*sr) == 0);
...
//  */
//
//   About = (aAbout)GetProcAddress( (HINSTANCE__*)DLLHandle, "FindNextStreamW");
//
...
// }
	...
	file = my::FindFirstStreamW_Linked( (*Path).c_str(), FindStreamInfoStandard, &stream, 0);
...
//						 ShowMessage((String)stream.cStreamName );
  ...
  if ( String(stream.cStreamName) == "::$DATA")                                 // jeœli to strumieñ pliku
		ListIt->ImageIndex = TypeFileIcon( Path , list, 0);                     // œcie¿ka do pliku przez którego strumieñ widaæ na liœcie
					 ...
					 ListIt->SubItems->Add( stream.StreamSize.QuadPart );                   // do póŸniejszego wyliczania i sortowania informacja w bajtach
...
//					S::T.Continue();
//					throw Exception("Przerwanie"); break;         //bug w¹tku
//			  }
			}

//		} while (FindNextStreamW(file, &stream) != 0);
	} while ( my::FindNextStreamW_Linked( (HANDLE)file, (void*)&stream) != 0);
...
unsigned short TypeFileIcon( String *path, TListView *list, bool ext) {           // ext only = ADS
	...
	unique_ptr<TIcon> h(new TIcon());
	static HANDLE ic = 0;                                                      	// zapamiêtuje ostanio u¿wywan¹ ikone
	HICON ico = 0;
	WORD idx = 0;

	static unsigned short e = 6;
...
//		if ( (*path)[(*path).Length()] == ":")
//					  ShowMessage(*path);
			if ( (*path).LastDelimiter(".") )									// jeœli jest kropka
		...
		h->Handle = ExtractAssociatedIcon( HInstance, (*path).c_str(), &idx );

		DeleteFileW( (*path).c_str() );                                     // nie sprz¹tam po sobie
	}

	else  h->Handle = ExtractAssociatedIcon( HInstance, (*path).c_str(), &idx );

			img->Picture->Icon = h.get();

			if ( ic != (*img).Picture->Icon->Handle ) {                         	// jeœli uchwyty do ikon s¹ ró¿ne dodaje ikonê (zapobiega dodawaniu siê tych samych ikon pod rz¹d)

				S::iconlist_large->AddIcon((*img).Picture->Icon);
				S::iconlist_small->AddIcon(  h.get() );
			}

			ic = (*img).Picture->Icon->Handle;		// dodajemy do historii

	return S::iconlist_large->Count-1;
}
...
void ShowError( AnsiString kom, Exception *exception, TObject *Sender) {
	...
	else (*showtimes)++;       //jeœli s¹ ró¿ne inkrementuje licznik showtimes;
...
/*IDTRYAGAIN*/ case 4 : *showtimes = 0; *lastkom = ""; break;                   // resetuje licznik, i ostatni b³¹d
...
AnsiString ErrorToText(int LastError, bool code, bool save) {                   // opisuje tylko wybrane kody b³êdów
	...
	switch (LastError) {
		...
		case 39: return "The disk is full" + *e;
		case 50: return "The request is not supported" + *e;
