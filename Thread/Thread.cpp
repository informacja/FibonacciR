...
__fastcall THsearch::~THsearch()  {
	...
	FibonacciR->ListView1->Visible = true;
	FibonacciR->ListView1->Show();
	FibonacciR->ButtonSzukaj->Enabled = true;
}

//---------------------------------------------------------------------------
TNotifyEvent __fastcall THsearch::OnbtClick(TObject *Sender)
{
...
//	while( !this->Terminated )	// Nieskoñczona pêtla dzia³ania w¹tku. Chyba, ¿e j¹ przerwiemy np. Terminate() - patrz funkcja niszcz¹ca w¹tek
	{    //, , *filtr.get()
		if (FibonacciR->SpeedMode1->Checked )
			FibonacciR->ListView1->Visible = false;
		TButton *bt = new TButton(FibonacciR);
		bt->Parent = FibonacciR;

		bt->SetParentComponent( FibonacciR->TabSheet1 );
		bt->Left = 10;
		bt->Top = 10;
		bt->Caption = "Stop";
		bt->Show();
		bt->OnClick = OnbtClick(NULL);
...
//		ShowMessage(*filtr.get()) ;
		...
		{
			if ( exception.ToString() == "Przerwanie") S::T.Show(FibonacciR->StatusBar1);		// do nothing
			else ShowError( "ButtonSzukajClick()", &exception, this);
		}
		catch (...) { ShowError("Nieznany b³¹d wyszukiwania TFibonacciR::ButtonSzukajClick()",0, this);}
