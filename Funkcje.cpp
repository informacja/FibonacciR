...
// ---------------------------------------------------------------------------
 String SearchStream(TListView *list, String Dir, String typ, unsigned n, unsigned char m) {
	...
	if( Dir.IsEmpty()) return "Œcie¿ka pusta";
...
//							 ShowMessage(*name);
	...
	if (Form2->CheckBoxHideData->Checked) ListIt->SubItems->Add( *name.get() ); // warunek musi byæ po list->Items->Add()

	else			 ListIt->SubItems->Add( stream.cStreamName);
								  LogSave(*name.get());						    // sprawdziæ czy wczeœciej niezeuje
			...
			if ( *n == FibonacciR->BREAK_SEARCH) {                              // zapytanie czy szukaæ nadal (wartoœæ)
	...
	if (ext) {
		...
		if ( *path == "::$DATA" ) return ICO_DATA;
			...
			*path = "TEMP" + GetExtension( *path,1);
			...
			// bez closehanle po zamkniêciu aplikacji automatycznie siê usun¹
...
//		 seconds = Frequency/1000;
		*mils = FloatToStrF(Frequency, ffFixed, 15, 15);                        // dok³adnoœæ do sekundo do -18 potêgi (attosecond)
...
//		(reinterpret_cast< TEdit *>		(Sender))->Text 	= Result();        	// dla odpowiedniego componentu wykona siê tylko jeden z poni¿szych
